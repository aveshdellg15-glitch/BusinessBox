...
function friendlyError(error){const message=error?.message||'Please try again.';return /schema cache|does not exist|Could not find the function/i.test(message)?'Workspace setup is incomplete. Please ask the site owner to run the supplied Supabase setup script. '+message:message;}
...
function fieldMarkup(field,row){
 ...
 if(type==='textarea')return `<label>${label}<textarea ${attrs}>${esc(value)}</textarea></label>`;
...
function openEntity(kind,index=null){
 ...
 openModal((row.id?'Edit ':'Add ')+def.label,`<form class="form" onsubmit="event.preventDefault();saveEntity()"><div class="pair">${def.fields.map(f=>fieldMarkup(f,row)).join('')}</div><p class="help">${kind==='invoice'?'This records an invoice; it does not collect payment or send email.':'Saved in '+esc(currentBusiness().name)+'.'}</p><div class="toolbar"><button class="btn primary" id="saveBtn">Save ${def.label.toLowerCase()}</button>${kind==='invoice'?'<button type="button" class="btn" onclick="printInvoice()">Print / Save PDF</button>':''}</div><div id="formMessage" class="message" role="status"></div></form>`);
}
function readEntity(){
 const row={};for(const [name,,type] of ENTITIES[editor.kind].fields){
  let value=$('field-'+name).value;
  if(type==='number'){value=Number(value);if(!Number.isFinite(value)||value<0)throw Error('Amounts must be zero or a positive number.');}
...
async function loadRecords(){
 ...
 $('recordTabs').innerHTML=Object.entries(ENTITIES).map(([key,d])=>`<button class="btn ${key===kind?'active':''}" onclick="showRecords('${key}')">${d.plural}</button>`).join('');
 $('recordRows').innerHTML='<tr><td colspan="5">Loading records…</td></tr>';
 const {data,count,error}=await db().from(def.table).select('*',{count:'exact'}).eq('business_id',id).order('created_at',{ascending:false}).order('id').range(page*PAGE_SIZE,(page+1)*PAGE_SIZE-1);
...
async function exportRecords(){
 ...
 try{
  const all=[];for(let offset=0;;offset+=500){const {data,error}=await db().from(def.table).select('*').eq('business_id',id).order('created_at').order('id').range(offset,offset+499);if(error)throw error;all.push(...data);if(data.length<500)break;}
...
function openDocument(kind,index=null){
 ...
 let fields='';
 if(kind==='quote')fields=`<div class="pair"><label>Customer<input id="docCustomer" value="${esc(payload.customer)}" required></label><label>Quote reference<input id="docReference" value="${esc(payload.reference)}"></label><label>Date<input id="docDate" type="date" value="${esc(payload.date)}"></label></div><p class="help">Description · Quantity · Rate (R). Enter your agreed rates; no tax is added automatically.</p><div id="quoteLines"></div><button type="button" class="btn" onclick="addQuoteLine()">+ Add service line</button><strong id="quoteTotal"></strong><label>Scope, exclusions and payment notes<textarea id="docNotes">${esc(payload.notes)}</textarea></label>`;
 if(kind==='checklist')fields=`<label>Edit tasks (one per line)<textarea id="docTasks" rows="9">${esc(payload.tasks.map(t=>t.text).join('\n'))}</textarea></label><button type="button" class="btn" onclick="syncChecklist()">Update task list</button><div id="checklistTasks"></div><label>Job / customer / notes<textarea id="docNotes">${esc(payload.notes)}</textarea></label>`;
 if(kind==='message')fields=`<div class="pair"><label>Message type<select id="docVariant"><option value="booking">Booking confirmation</option><option value="followup">Quote follow-up</option><option value="review">Feedback / review request</option></select></label><label>Customer<input id="docCustomer" value="${esc(payload.customer)}"></label><label>Service<input id="docService" value="${esc(payload.service)}"></label><label>Agreed date and time<input id="docDate" value="${esc(payload.date)}"></label></div><button class="btn" type="button" onclick="regenerateMessage()">Use these details in a fresh draft</button><label>Edit your message<textarea id="docText" rows="10">${esc(payload.text)}</textarea></label><p class="help">Replace any bracketed details, then copy or download to send it yourself.</p>`;
 openModal(existing?'Edit saved '+kind:'Create '+kind,`<form class="form" onsubmit="event.preventDefault();saveDocument()"><label>Document title<input id="docTitle" required maxlength="160" value="${esc(title)}"></label>${fields}<div class="toolbar"><button id="saveBtn" class="btn primary">Save document</button><button type="button" class="btn" onclick="downloadDocument()">Download text</button><button type="button" class="btn" onclick="printDocument()">Print / Save PDF</button>${kind==='message'?'<button type="button" class="btn" onclick="copyDocument()">Copy message</button>':''}</div><div id="formMessage" class="message" role="status"></div></form>`);
 if(kind==='quote')renderQuoteLines();
 if(kind==='checklist')renderChecklist();
 if(kind==='message')$('docVariant').value=payload.variant;
}
function renderQuoteLines(){
 $('quoteLines').innerHTML=documentEditor.payload.items.map((item,i)=>`<div class="quote-line"><input aria-label="Service ${i+1}" id="quote-description-${i}" value="${esc(item.description)}"><input aria-label="Quantity ${i+1}" id="quote-quantity-${i}" type="number" min="0" step="0.01" value="${esc(item.quantity)}" oninput="updateQuoteTotal()"><input aria-label="Rate ${i+1}" id="quote-rate-${i}" type="number" min="0" step="0.01" value="${esc(item.rate)}" oninput="updateQuoteTotal()"><button class="btn danger" type="button" aria-label="Remove service ${i+1}" onclick="removeQuoteLine(${i})">×</button></div>`).join('');updateQuoteTotal();
}
...
function renderChecklist(){$('checklistTasks').innerHTML=documentEditor.payload.tasks.map((task,i)=>`<label class="check-row"><input type="checkbox" ${task.done?'checked':''} onchange="documentEditor.payload.tasks[${i}].done=this.checked"><span>${esc(task.text)}</span></label>`).join('');}
...
async function loadDocuments(){
 ...
 const {data,count,error}=await db().from('business_documents').select('*',{count:'exact'}).eq('business_id',id).order('updated_at',{ascending:false}).order('id').range(page*PAGE_SIZE,(page+1)*PAGE_SIZE-1);
 ...
 $('documentList').innerHTML=documentRows.length?documentRows.map((row,i)=>`<div class="section-head"><div><strong>${esc(row.title)}</strong><p class="help">${esc(row.kind)} · ${esc(String(row.updated_at).slice(0,10))}</p></div><div class="toolbar"><button class="btn" onclick="openDocument(null,${i})">Open / edit</button><button class="btn danger" onclick="deleteDocument(${i})">Delete</button></div></div>`).join(''):'No saved documents yet. Create a quote, checklist or customer message above.';
